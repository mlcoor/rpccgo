...
// Protocol selection: explicit via context, or fallback order: connectrpc
func TestService_Ping(ctx context.Context, req *PingRequest) (*PingResponse, error) {
	...
	if h, ok := rpcruntime.LookupConnectHandler("cgotest.TestService"); ok {
		if svc, ok := h.(TestServiceHandler); ok {
			return svc.Ping(ctx, req)
		}
	}
	return nil, rpcruntime.ErrServiceNotRegistered
